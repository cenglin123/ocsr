...
>
> OCSR 的可用模型由仓库根目录的 [`config/allowed-models.json`](../config/allowed-models.json) 唯一决定。默认值是 `xiaomi/mimo-v2.5` 与 `xiaomi/mimo-v2.5-pro`；用户可修改该非空、无重复的 JSON 字符串数组，下一次命令启动时加载。未列出的模型不可选。
